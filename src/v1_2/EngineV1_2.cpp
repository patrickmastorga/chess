...
void EngineV1_2::inputMove(const StandardMove& move)
{
    ...
    for (Move& legalMove : enginePositionMoves) {
        if (legalMove == move) {
            ...
            rootTotalHalfmoves = totalHalfmoves;
...
int32 EngineV1_2::search_std(uint8 plyFromRoot, uint8 depth, Move* moveStack, uint32 startMoves, int32 alpha, int32 beta, uint32 &nodesSearched)
{
    ++nodesSearched;
    // BASE CASES
...
int32 EngineV1_2::search_quiscence(Move* moveStack, uint32 startMoves, int32 alpha, int32 beta, uint32 &nodesSearched)
{
    ++nodesSearched;
...
EngineV1_2::MoveOrderer::Iterator EngineV1_2::MoveOrderer::end()
{
    return Iterator(moveStack + startMoves, endMoves - startMoves, endMoves - startMoves);
}

EngineV1_2::MoveOrderer::Iterator::Iterator(Move* start, uint32 size, uint32 currentIndex) : start(start), size(size), idx(currentIndex) {}

EngineV1_2::Move& EngineV1_2::MoveOrderer::Iterator::operator*()
{
    return start[idx];
}
...
bool EngineV1_2::MoveOrderer::Iterator::operator!=(const Iterator& other) const
{
